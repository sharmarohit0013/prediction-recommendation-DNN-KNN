...
app.post('/api/inputData',(req,res) => {
	...
	const pythonProcess = spawn('python',["../Keras-NN-KNN-Model-Code/enforce.py",InputData.nodeInput[0],InputData.nodeInput[1],InputData.nodeInput[2],InputData.nodeInput[3],InputData.id]);
	...
	pythonProcess.stdout.on('data', (data) => 
	{
		...
		{
			console.log("recived Valid JSON String from spawned process");
...
{
	console.log("new Recommendation For Incorrect Solution");
	const InputData = {
        nodeInput: req.body.nodeInput,		
		alarmCondition:	req.body.alarmCondition,
...
function ModelRetrain(InputData, res) {
	var result = "Failed";
	const spawn = require("child_process").spawn;
	const pythonProcess = spawn('python',["../Keras-NN-KNN-Model-Code/NN-Trainer.py"]);
	console.log("NN-Trainer spawned ")
	pythonProcess.stdout.on('data', (data) => 
	{		
		var strr = data.toString('utf8');//convert buffer to string
		var replaced = strr.split("'").join('"');//replace ' with " for proper json formet
		...
		if(result === "Failed"){
			console.log("Retrain Model Status = "+ result);
	...
	pythonProcess.stderr.on('data', function(data) {
   		console.log('stderr: ' + data);
		//Here is where the error output goes
		console.log("retrain Model Status2 = "+ result);
		res.send({"retrainModelStatus":result,"reasonForError":"Error while Retraining Model"});
	});
